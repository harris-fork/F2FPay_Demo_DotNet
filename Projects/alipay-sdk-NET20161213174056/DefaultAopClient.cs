...
namespace Aop.Api
{
    ...
    public class DefaultAopClient
    {
        ...
        public T pageExecute<T>(IAopRequest<T> request, string accessToken, string reqMethod) where T : AopResponse
        {
            ...
            txtParams.Add(TERMINAL_TYPE, request.TerminalType);
            txtParams.Add(TERMINAL_INFO, request.TerminalInfo);
            txtParams.Add(PROD_CODE, request.ProdCode);
        ...
        private AopDictionary buildRequestParams<T>(IAopRequest<T> request, String accessToken, String appAuthToken) where T : AopResponse
        {
            ...
            // 获取参数
            String charset = String.IsNullOrEmpty(this.charset) ? "utf-8" : this.charset;
            String apiVersion = String.IsNullOrEmpty(request.ApiVersion) ? this.Version : request.ApiVersion;
            ...
            result.Add(TERMINAL_TYPE, request.TerminalType);
            result.Add(TERMINAL_INFO, request.TerminalInfo);
            result.Add(PROD_CODE, request.ProdCode);
            result.Add(NOTIFY_URL, request.NotifyUrl);
